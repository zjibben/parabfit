...
!! HYPRE_FGMRES_TYPE
!!
!! This module defines a FGMRES solver class built on the ParCSR FlexGMRES
!! solver from Hypre that uses BoomerAMG as the preconditioner.
!!
...
module hypre_fgmres_type
  ...
  contains
    ...
    final :: hypre_fgmres_delete
  end type hypre_fgmres

contains

  !! Final subroutine for HYPRE_FGMRES objects.
  subroutine hypre_fgmres_delete (this)
    type(hypre_fgmres) :: this
    integer :: ierr
    ierr = 0
    ...
    if (hypre_associated(this%solver)) call fHYPRE_FlexGMRESDestroy (this%solver, ierr)
    if (hypre_associated(this%precon)) call fHYPRE_BoomerAMGDestroy (this%precon, ierr)
    INSIST(ierr == 0)
  end subroutine hypre_fgmres_delete

  subroutine init (this, A, params)

    class(hypre_fgmres), intent(out) :: this
    ...
    call params%get ('max Krylov vectors', this%k_dim, default=this%max_iter)
    INSIST(this%k_dim > 0)
  ...
  function matrix (this)
    class(hypre_fgmres), intent(in) :: this
  ...
  subroutine get_metrics (this, num_itr)
    ...
    if (present(num_itr)) then
      call fHYPRE_FlexGMRESGetNumIterations (this%solver, num_itr, ierr)
      INSIST(ierr == 0)
    end if
  ...
  subroutine compute (this)
    ...
    class(hypre_fgmres), intent(inout) :: this
    ...
    !! Create the Hypre FlexGMRES solver object. This supposes that once the solver
    ...
    if (hypre_associated(this%solver)) call fHYPRE_FlexGMRESDestroy (this%solver, ierr)
    call fHYPRE_FlexGMRESCreate (this%solver, ierr)
    INSIST(ierr == 0)

    !! Set the FlexGMRES parameters.
    call fHYPRE_FlexGMRESSetPrintLevel (this%solver, this%print_level, ierr)
    call fHYPRE_FlexGMRESSetTol (this%solver, this%err_tol, ierr)
    call fHYPRE_FlexGMRESSetAbsoluteTol (this%solver, 0.0_r8, ierr)
    call fHYPRE_FlexGMRESSetMaxIter (this%solver, this%max_iter, ierr)
    INSIST(ierr == 0)

    !! Initialize the Hypre solution and RHS vectors;
    !! structure (not values) needed to setup FlexGMRES solver.
    ...
    call fHYPRE_FlexGMRESSetPrecond (this%solver, this%precon, ierr)
    call fHYPRE_FlexGMRESSetup (this%solver, this%Ah, this%bh, this%xh, ierr)
    INSIST(ierr == 0)
  ...
  subroutine solve (this, b, x)
    ...
    !! Solve the system.
    call fHYPRE_FlexGMRESSolve (this%solver, this%Ah, this%bh, this%xh, ierr)
    INSIST(ierr == 0)
