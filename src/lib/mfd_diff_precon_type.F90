...
!! for a local mimetic finite difference diffusion matrix.
!!
...
!! substitution.
!!
!! The derived type MFD_DIFF_PRECON has the following type bound procedures.
!!
...
!!    MFD_DIFF_MATRIX.  The object takes ownership of DM by moving its
...
!!
!!      'preconditioner'  - The choice of preconditioner for the face Schur
...
!!
!!  MATRIX() returns a pointer to the MFD_DIFF_MATRIX object DM with which the
...
module mfd_diff_precon_type
  ...
  type, public :: mfd_diff_precon
    ...
    type(csr_matrix), pointer :: Sff => null()  ! needs the target attribute
    class(csr_precon), allocatable :: Sff_precon
  contains
    procedure :: init
    procedure :: compute
    procedure :: apply
  ...
  end type mfd_diff_precon

contains

  !! Final subroutine for MFD_DIFF_PRECON objects
  subroutine mfd_diff_precon_delete (this)
    type(mfd_diff_precon) :: this
    if (associated(this%Sff)) deallocate(this%Sff)
  end subroutine mfd_diff_precon_delete

  subroutine init (this, dm, params)

    use csr_precon_ssor_type
    use csr_precon_boomer_type
    use logging_services

    class(mfd_diff_precon), intent(out) :: this
    ...
    select case (method)
    case ('SSOR')
      allocate(csr_precon_ssor :: this%Sff_precon)
    case ('BoomerAMG')
      allocate(csr_precon_boomer :: this%Sff_precon)
    case default
      call LS_fatal (context//'unknown "method": '//method)
    end select
    if (params%is_sublist('parameters')) then
      plist => params%sublist('parameters')
      call this%Sff_precon%init (this%Sff, plist)
    else
      call LS_fatal (context//'missing "parameters" sublist parameter')
    end if
  ...
  subroutine compute (this)
    ...
    call start_timer ('CSR-precon-compute')
    call this%Sff_precon%compute
    call stop_timer ('CSR-precon-compute')
