...
!!
!!  This module defines the derived type CSR_PRECON_BOOMER which is an
!!  extension of the abstract base class CSR_PRECON that uses the BoomerAMG
...
module csr_precon_boomer_type
  ...
  type, extends(csr_precon), public :: csr_precon_boomer
    private
  ...
  end type csr_precon_boomer

contains

  !! Final subroutine for CSR_PRECON_BOOMER objects.
  subroutine csr_precon_boomer_delete (this)
    type(csr_precon_boomer) :: this
    integer :: ierr
    ierr = 0
    ...
    INSIST(ierr == 0)
  end subroutine csr_precon_boomer_delete
  ...
  subroutine init (this, A, params)
    ...
    if (this%max_iter <= 0) call LS_fatal (context//'"num-cycles" must be > 0')
    call params%get('print-level', this%print_level, default=0, stat=stat, errmsg=errmsg)
    if (stat /= 0) call LS_fatal (context//errmsg)
    if (this%print_level < 0 .or. this%print_level > 3) call LS_fatal (context//'"print-level" must be >= 0 and <= 3')
    call params%get('debug-level', this%debug_level, default=0, stat=stat, errmsg=errmsg)
    if (stat /= 0) call LS_fatal (context//errmsg)
    if (this%debug_level < 0) call LS_fatal (context//'"debug-level" must be >= 0')
    call params%get('logging-level', this%logging_level, default=0, stat=stat, errmsg=errmsg)
    if (stat /= 0) call LS_fatal (context//errmsg)
    if (this%logging_level < 0) call LS_fatal (context//'"logging-level" must be >= 0')
