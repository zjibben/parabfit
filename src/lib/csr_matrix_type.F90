...
!! CSR_MATRIX_TYPE
!!
!! This module defines data structures for sparse matrices stored in compressed
!! sparse row (CSR) format.  The implementation is limited to square matrices
!! with a symmetric non-zero structure, but general non-symmetric values.
!!
...
!!
!! This module defines the derived types CSR_MATRIX and CSR_GRAPH that
!! together describe a sparse matrix using the CSR format.  A CSR_GRAPH object
!! describes the symmetric non-zero structure of a sparse matrix.  A CSR_MATRIX
!! object composes a CSR_GRAPH object with the corresponding non-zero matrix
!! values to complete the description of a sparse matrix.  Multiple CSR_MATRIX
!! objects can share the same CSR_GRAPH object.
!!
!! The derived type CSR_GRAPH has the following type bound procedures.  The
...
!!
!!  INIT (NROW) initializes the graph object; NROW is the number of rows and
!!    columns of the matrix.  This must be the first call.
!!
...
!!
!! The derived type CSR_MATRIX is initialized using one of the following type
...
!!    a pointer to a CSR_GRAPH object; GRAPH%DEFINED() must return true.  If
...
!!    by the CSR_MATRIX object MOLD.  The matrix object regards the graph as a
...
!! data components of the object: if ARRAY is a CSR_MATRIX object then
...
module csr_matrix_type
  ...
  contains
    ...
    procedure, private :: csr_graph_add_edge_one
    procedure, private :: csr_graph_add_edge_many
    generic   :: add_edge => csr_graph_add_edge_one, csr_graph_add_edge_many
    procedure :: add_clique => csr_graph_add_clique
    procedure :: add_complete => csr_graph_add_complete
  ...
  contains
    procedure, private :: csr_matrix_init
    procedure, private :: csr_matrix_init_mold
    generic   :: init => csr_matrix_init, csr_matrix_init_mold
    procedure :: graph_ptr => csr_matrix_graph
    procedure :: set_all => csr_matrix_set_all
    procedure :: set => csr_matrix_set
    procedure :: increment => csr_matrix_increment
...
contains
  ...
  subroutine csr_graph_init (this, nrow)
    ...
    this%nrow = nrow
    allocate(this%g)
    call this%g%init (nrow, self_edge=.true.)
  end subroutine csr_graph_init

  subroutine csr_graph_add_clique (this, indices)
    class(csr_graph), intent(inout) :: this
    integer, intent(in) :: indices(:)
    ASSERT(allocated(this%g))
    call this%g%add_clique (indices)
  end subroutine csr_graph_add_clique

  subroutine csr_graph_add_edge_one (this, row, col)
    class(csr_graph), intent(inout) :: this
    integer, intent(in) :: row, col
    ASSERT(allocated(this%g))
    call this%g%add_edge (row, col)
  end subroutine csr_graph_add_edge_one

  subroutine csr_graph_add_edge_many (this, row, col)
    class(csr_graph), intent(inout) :: this
    integer, intent(in) :: row, col(:)
    ASSERT(allocated(this%g))
    call this%g%add_edge (row, col)
  end subroutine csr_graph_add_edge_many

  subroutine csr_graph_add_complete (this)
    class(csr_graph), intent(inout) :: this
    ASSERT(allocated(this%g))
    call this%g%get_adjacency (this%xadj, this%adjncy)
    deallocate(this%g)
  end subroutine csr_graph_add_complete

  logical function csr_graph_defined (this)
    class(csr_graph), intent(in) :: this
    csr_graph_defined = allocated(this%xadj)
  end function csr_graph_defined
  ...
  end subroutine csr_matrix_delete

  !! Initialize the sparse matrix with non-zero structure GRAPH.
  subroutine csr_matrix_init (this, graph, take_graph)
  ...
  end subroutine csr_matrix_init

  !! Initialize the sparse matrix using the non-zero structure from MOLD.
  subroutine csr_matrix_init_mold (this, mold)
    class(csr_matrix), intent(out) :: this
    class(csr_matrix), intent(in)  :: mold
    call csr_matrix_init (this, mold%graph, take_graph=.false.)
  end subroutine csr_matrix_init_mold

  !! Return a pointer to the sparse matrix graph.
  function csr_matrix_graph (this) result (graph)
    class(csr_matrix), intent(in) :: this
    type(csr_graph), pointer :: graph
    graph => this%graph
  end function csr_matrix_graph

  !! Set all matrix elements to the given value.
  subroutine csr_matrix_set_all (this, value)
    class(csr_matrix), intent(inout) :: this
    real(r8), intent(in) :: value
    ASSERT(allocated(this%values))
    this%values = value
  end subroutine csr_matrix_set_all

  !! Set the specified matrix element to the given value.
  subroutine csr_matrix_set (this, row, col, value)
    class(csr_matrix), intent(inout) :: this
    integer, intent(in) :: row, col
    real(r8), intent(in) :: value
    integer :: n
    ...
    ASSERT(n /= 0)
    this%values(n) = value
  end subroutine csr_matrix_set

  !! Increment the specified matrix element by the given value.
  subroutine csr_matrix_increment (this, row, col, value)
    class(csr_matrix), intent(inout) :: this
    integer, intent(in) :: row, col
    real(r8), intent(in) :: value
    integer :: n
  ...
  end subroutine csr_matrix_increment

  !! Zero-out the values of the specified row and column elements.
  subroutine csr_matrix_project_out (this, index)
    class(csr_matrix), intent(inout) :: this
  ...
  end subroutine csr_matrix_project_out

  !! Return pointers to the values and corresponding column indices
  !! of the specified sparse matrix row.
  subroutine csr_matrix_get_row_view (this, row, values, indices)
    class(csr_matrix), intent(in), target :: this
  ...
  end subroutine csr_matrix_get_row_view
  ...
  subroutine csr_matrix_get_diag_copy (this, diag)
    class(csr_matrix), intent(in) :: this
    real(r8), intent(inout) :: diag(:)
    integer :: i, k
    ASSERT(size(diag) >= this%nrow)
    do i = 1, this%nrow
  ...
  pure integer function array_index (graph, row, col) result (k)
    type(csr_graph), intent(in) :: graph
    integer, intent(in) :: row, col
