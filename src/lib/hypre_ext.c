...
int
HYPRE_Ext_ParCSRFlexGMRESCreate(HYPRE_Solver *solver)
{
  return HYPRE_ParCSRFlexGMRESCreate(MPI_COMM_WORLD, solver);
}

/* NNC, May 2014: I've followed the pattern above for PCGSetup and PCGSolve.
 * The same questions posed there apply here too. */

int
HYPRE_Ext_FlexGMRESSetup(HYPRE_Solver solver, HYPRE_IJMatrix A, HYPRE_IJVector b, HYPRE_IJVector x)
{
  HYPRE_ParCSRMatrix par_A;
  HYPRE_ParVector par_b, par_x;
  HYPRE_IJMatrixGetObject(A, (void**) &par_A);
  HYPRE_IJVectorGetObject(b, (void**) &par_b);
  HYPRE_IJVectorGetObject(x, (void**) &par_x);
  return HYPRE_ParCSRFlexGMRESSetup(solver, par_A, par_b, par_x);
}

int
HYPRE_Ext_FlexGMRESSolve(HYPRE_Solver solver, HYPRE_IJMatrix A, HYPRE_IJVector b, HYPRE_IJVector x)
{
  HYPRE_ParCSRMatrix par_A;
  HYPRE_ParVector par_b, par_x;
  HYPRE_IJMatrixGetObject(A, (void**) &par_A);
  HYPRE_IJVectorGetObject(b, (void**) &par_b);
  HYPRE_IJVectorGetObject(x, (void**) &par_x);
  return HYPRE_ParCSRFlexGMRESSolve(solver, par_A, par_b, par_x);
}

int
HYPRE_Ext_FlexGMRESSetBoomerAMGPrecond(HYPRE_Solver solver, HYPRE_Solver precond)
{
  return HYPRE_ParCSRFlexGMRESSetPrecond(solver, HYPRE_BoomerAMGSolve, HYPRE_BoomerAMGSetup, precond);
}
