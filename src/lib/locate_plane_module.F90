...
type :: interface_flux
  real(r8) :: Flux_Vol                 ! The flux volume
  real(r8) :: Flux_Vol_Coord(ndim,nvc) ! The physical coordinates of the flux volume vertices of the interface cells
  integer :: Face                      ! The face identifier for this advection sweep
  real(r8) :: Advected_Volume          ! The advected volume of the materials behind this interface
  integer :: Iter                      ! The iteration count required to locate this plane
contains
  procedure :: init => setup_interface_flux
end type interface_flux

type :: interface_geometry
  type(locate_plane_hex), dimension(:), allocatable :: cell
  !type(truncvol_data), dimension(:,:), pointer :: trunc_vol
contains
  procedure :: init => setup_interface_geom
  ! procedure, private :: truncate_volume
  ! procedure, private :: truncate_face
  ! procedure, private :: face_param
end type interface_geometry

type, public :: interface_data
  integer                                             :: nicells
  type(interface_geometry)                            :: geom
  type(interface_flux)    , dimension(:), allocatable :: flux
contains
  !procedure :: init
  !procedure          :: unit_test
  !procedure          :: locate_plane
...
! ! interfaces for functions lying in truncate_volume_module
! interface
!    subroutine face_param (this, option, face)
!      type(interface_geometry), intent(inout) :: this
!      character(8), intent(in) :: option
!      integer     , intent(in) :: face
!    end subroutine face_param
!    function truncate_volume (this)
!      type(interface_geometry), intent(inout) :: this
!      real(r8), dimension(this%nicells) :: truncate_volume
!    end function truncate_volume
!    function truncate_face (this, f) result(Vf)
!      type(interface_geometry), intent(in) :: this
!      integer, intent(in) :: f
!      real(r8), dimension(this%nicells) :: Vf
...
!
!=======================================================================
subroutine setup_interface_geom (this, nicells, Mask, grad, Vof, mesh)
  use unstr_mesh_type

  class(interface_geometry), intent(out) :: this
  integer,                   intent(in)  :: nicells
  type(unstr_mesh),          intent(in)  :: mesh
  logical,                   intent(in)  :: Mask(mesh%ncell)
  real(r8),                  intent(in)  :: Grad(3,mesh%ncell)
  real(r8),                  intent(in)  :: Vof(mesh%ncell)

  integer :: n, v

  allocate(this%cell(nicells))

  ! Pack the interface normal vector.
  this%cell%Normal(1) = pack (grad(1,:), Mask)
  this%cell%Normal(2) = pack (grad(2,:), Mask)
  this%cell%Normal(3) = pack (grad(3,:), Mask)

  ! Pack the donor volume fraction.
  this%cell(:)%Vof = pack(Vof, Mask)

  ! The correct donor cell volume was put into Volume
  ! in the code above.  Just pack it into the arrays.
  this%cell%Volume = pack (mesh%Volume, Mask)

  ! Gather the donor cell's vertices
  do n = 1,ndim
    do v = 1, nvc
      this%cell(:)%node(n,v) = pack (mesh%x(n,mesh%cnode(v,:)), mask)
    end do
  end do
...
SUBROUTINE RHO_BRACKET (this, Rho_Min, Rho_Max, V_Min, V_Max, trunc_vol)
  ...
  FACE_LOOP: do f = 1,nfc
    ...
    call FACE_PARAM (this, 'full_cell', f, trunc_vol)
