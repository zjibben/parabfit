...
contains
  ...
  subroutine vof_init_circle(mesh, R, vof)
    ...
    do i = 1,mesh%ncell
      vof(1,i) = cell_vof(mesh%x(:,mesh%cnode(:,i)), R)
      vof(2,i) = 1 - vof(1,i)
    end do
  ...
  real(r8) function cell_vof(x,R)
    ...
    is_inside(2) = isInside([xb(1,1), xb(2,2)], R)
    is_inside(3) = isInside(xb(:,2), R)
    is_inside(4) = isInside([xb(1,2), xb(2,1)], R)
  ...
  pure real(r8) function antiderivative(x, R)
    real(r8), intent(in) :: x, R
    antiderivative = (R * sqrt(R**2 - x**2) - x**2 * log(sqrt(R**2 - x**2) + R)) / 2
